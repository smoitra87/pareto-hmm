...
\setlength{\unitlength}{0.3cm}
\begin{picture}(40,20)(0,0)
\multiput(7,15)(8,0){1}{\circle{4}}
\multiput(3,7)(8,0){1}{\circle{4}}
\multiput(3,7)(8,0){1}{\circle{3.7}}
\multiput(11,7)(8,0){1}{\circle{4}}
\multiput(11,7)(8,0){1}{\circle{3.7}}
\thicklines
...
\multiput(7,13)(8,0){1}{\line(1,-1){4}}
\multiput(6,7)(1,0){3}{\circle*{0.3}}
\put(24,15){\line(1,0){4}}
\put(30,13){\line(-1,-1){4}}
\put(30,13){\line(1,-1){4}}
\put(30,13){\line(0,-1){4}}
\multiput(29,7)(1,0){3}{\circle*{0.3}}
\thinlines
\put(30,15){\circle{4}}
\put(26,7){\circle{4}}
\put(26,7){\circle{3.7}}
...
In general, the convex pareto-frontier contains high quality sequences in itself. The convex hull contains sequences that are solutions for all values of $\theta$ for the standard linear multistate design problem $\argmin_X \theta E_A(X)+(1-\theta)E_B(X)$. However, there do lie undominated points in the concavities that maybe worthwhile to explore. Fortunately, there exists a way to explore the concavities as shown previously in a different context by Zheng et.al~\cite{Zheng2009}. The authors apply the convex hull procedure to local sections of the frontier by freezing entities in their dynamic programming problem. We modify their approach in the context of dynamic programming for TMRFs. Basically, the idea is to apply the Pareto-frontier method while keeping a residue to fixed to a certain amino acid. Since max-decoding can still be performed while keeping a node fixed this procedure is feasible. We then iterate over all amino acids and over all residues while performing max-decoding while culling dominated points. This will significantly explore the concavities. Keeping two or more residues fixed will further explore the space but an additional cost. 



\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{pics/convexfront3.pdf}
    \caption{This figure illustrates the point that there might be points in the interior of the convex frontier found by the algorithm that are not strictly dominated. P1 and P2 are on the convex frontier and the dashed magenta line is the convex hull. Any point in D1 is strictly dominated by P1 and any point in D2 is strictly dominated by P2. Therefore the points P3 and P4 are dominated by P1 and P2. The point P5 on the other hand is not dominated by P1 or P2 and yet is not explored by the algorithm. It does not lie on the convex frontier but it may lie on the pareto frontier. This is because P5 lies in the concavities of the sequence space.}
    \label{fig:cvxfront3}
\end{figure}
\pagebreak
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{pics/sim2_debug1.pdf}
