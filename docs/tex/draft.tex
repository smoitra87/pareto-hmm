...
The Quick hull procedure is used to find all the points on the pareto optimal frontier. The quick hull is traditionally used to find the convex hull of a set of points. In our case we are only interested in the lower left convex frontier. The algorithm in pseudo code is described in Algorithm~\ref{algo:quickhull1}.
...
For multi-state protein design we combine the energy functions as follows. This has the interesting propertu
...
\put(15,16.5){\footnotesize{$\psi(X_2{,}X_3)$}}
\put(22.5,16.5){\footnotesize{$\psi(X_{n{-}1}{,}X_n)$}}
\put(5.2,10.6){\footnotesize{$\psi(X_1{,}O^A_1)$}}
\put(13.2,10.6){\footnotesize{$\psi(X_2{,}O^A_2)$}}
\put(30.2,10.6){\footnotesize{$\psi(X_n{,}O^A_n)$}}
\put(5.2,18.6){\footnotesize{$\psi(X_1{,}O^B_1)$}}
\put(13.2,18.6){\footnotesize{$\psi(X_2{,}O^B_2)$}}
\put(30.2,18.6){\footnotesize{$\psi(X_n{,}O^B_n)$}}
\end{picture}
\label{fig:CMRF2}
\caption{A Tree structured Markov Random Field that is used for modelling multi-state protein design.}
\end{figure}

\pagebreak

\subsection{Convex Hull Procedure on CMRFs}
